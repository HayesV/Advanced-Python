...
#takes an interable object and creates a new sequence of sorted data
...
#modifies existing dataset instead of returning a new copy
dataset.sort(key=lambda d:d['snow'], reverse=False)
#pprint.pp(dataset)

# TODO: Sort on multiple fields: first by snowfall, then by average wind speed
multipleSorts = sorted(dataset, key=lambda d:(d['snow'], d['awnd']))
pprint.pp(multipleSorts)
