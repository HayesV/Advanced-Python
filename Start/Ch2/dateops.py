...
weekendDays = list(filter(isWeekend, weatherdata))
warmestDay = max(weekendDays, key=lambda d:d['tmax'])

print(date.fromisoformat(warmestDay['date']).strftime('%a, %d %b %Y'))
...
avgTemp = 0.0
nextDate = coldest_date

for _ in range(7):
    nextDate += timedelta(days=1)
    wd = next((day for day in weatherdata if day['date'] == str(nextDate)),None)
    avgTemp += (wd['tmin'] + wd['tmax']) / 2

avgTemp = avgTemp / 7
print(f"The average temp for the next 7 days was {avgTemp}")